...
from src.dqn_efficient import QNetwork, linear_schedule
from src.create_env import make_fovea_cv_env
...
def train(config):
    ...
    while global_transitions < config["algorithm"]["total_timesteps"]:
        ...
        # NOTE: We arent' really using the sensory actions values
        sensory_actions = np.array([random.randint(0, 1)])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/dqn_fovea_pong_motor_separate_lca_config.yml",
