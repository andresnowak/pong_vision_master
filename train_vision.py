...
def main(config):
    ...
    # we use a different seed for the evaluation (so as to not test in the same environment)
    eval_env = make_full_vision_env(
        config["env_id"], config["environment"]["n_envs_eval"], config["seed"] + 10
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/dqn_full_vision_pong_config.yml", # Default config file name
