...
def train(config, vision_model):
    ...
    # Make network
    vision_q_network = QNetwork(envs, sensory_action_set=sensory_action_set).to(device)
    vision_q_network.load_state_dict(vision_model)
    for param in vision_q_network.parameters():
        param.requires_grad = False
    vision_q_network.eval()

    motor_q_network = QNetwork(envs).to(device)
    optimizer = optim.Adam(
        motor_q_network.parameters(), lr=config["algorithm"]["learning_rate"]
    )
    target_network = QNetwork(envs).to(device)
    target_network.load_state_dict(motor_q_network.state_dict())
    ...
    while global_transitions < config["algorithm"]["total_timesteps"]:
        ...
        if global_transitions % config["algorithm"]["train_frequency"] == 0:
            ...
            if global_transitions > config["algorithm"]["learning_starts"]:
                with torch.no_grad():
                    motor_target = target_network(
                        resize(data.next_observations)
                    )
                    motor_target_max, _ = motor_target.max(dim=1) # we grab the biggest Q value (Remember from the Q value function)
                ...
                old_motor_q_val = motor_q_network(
                ...
                old_motor_val = old_motor_q_val.gather(
                    1, data.actions
                ).squeeze()

                # 3. Calculate Loss
                # The loss is between the TD target and the Q-value of the action taken
                loss = F.mse_loss(td_target, old_motor_val)
                    ...
                    wandb.log(
                        {
                            ...
                            "sensor_q_values": old_motor_val.mean().item(),
                    ...
                    while not done:
                        ...
                        if done:
                            eval_episodic_returns.append(
                                infos["final_info"][0]["reward"]  # to use our custom reward
                            )
                            ...
                            if eval_ep == 0:
                                ...
                                torch.save(
                                    {
                                        "motor_q": motor_q_network.state_dict(),
                                        "vision_q": vision_q_network.state_dict(),
                                    },
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/dqn_fovea_peripheral_pong_vision_separate_config.yml",
    ...
    vision_model = torch.load(config["paths"]["vision_model_path"])["q"]

    train(config, vision_model)
