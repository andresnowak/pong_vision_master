...
class CVFidexFovealEnv(FixedFovealEnv):
    ...
    def step(self, action):
        ...
        sensory_action = (
            action["sensory_action"] if sensory_action is None else sensory_action
        )
    ...
    def detect_ball_position(self, observation):
        ...
        for contour in contours:
            ...
            if (
                0.0 <= area < 10
            ):  # The area of the ball seems to be 3 (but it says w and h are 2 and 4 of the bounding rect, so seems strange to me)
                ...
                if 0.2 < aspect_ratio < 2.5:
                    ...
                    candidates.append(
                        [y + h // 2, x + w // 2]
                    )  # The values are row and column, so we want y and x not x and y
...
class CVFidexFovealPeripheralEnv(FixedFovealPeripheralEnv):
    def __init__(self, env, args):
        super().__init__(env, args)
...
def CVAtariFixedFovealPeripheralEnv(args: AtariEnvArgs) -> gym.Wrapper:
    base_env = AtariBaseEnv(args)
    wrapped_env = CVFidexFovealPeripheralEnv(base_env, args)
    return wrapped_env
